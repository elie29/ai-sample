...
- its scope - the files it is allowed to touch, one component when possible;
- its verification command (`npm test`, `npm run lint`, `npm run build`);
...
Tick a task only after its verification command has actually been run and the screen has
been looked at in the browser.
